...
也接受 `!訂閱`、`!取消訂閱`、`!subscribe`、`!unsubscribe` 等別名；團體可輸入 `日向`、`hinata`、`櫻坂` 或 `sakura`。輸入 `小坂菜緒` 與 `小坂 菜緒` 會視為同一位成員。日向坂吉祥物的官方部落格作者名稱為 `ポカ`，訂閱時也接受 `ぽか` 或不分大小寫的 `Poka`。一次處理多位作者時，可使用 `、`、半形／全形逗號、分號或換行分隔；批次中重複輸入的名字只會處理一次，找不到的名字會單獨列在回覆中，不影響其他有效作者。
