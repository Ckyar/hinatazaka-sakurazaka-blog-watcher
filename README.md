...
也接受 `!訂閱`、`!取消訂閱`、`!subscribe`、`!unsubscribe` 等別名；團體可輸入 `日向`、`hinata`、`櫻坂` 或 `sakura`。輸入 `小坂菜緒` 與 `小坂 菜緒` 會視為同一位成員。一次處理多位成員時，可使用 `、`、半形／全形逗號、分號或換行分隔；批次中重複輸入的名字只會處理一次，找不到的名字會單獨列在回覆中，不影響其他有效成員。
